    ...
    public class FileDownloader
    {
        ...
        public void Download(string url, string destFile)
        {
            ...
            try
            {
                ...
                data = DownloadData.Create(url, destFile, this.proxy);
                ...
                //destFile = destFolder.Replace("file:///", "").Replace("file://", "");
                this.downloadingTo = destFile;
                ...
                if (!System.IO.File.Exists(downloadingTo))
                {
                    FileStream fs = System.IO.File.Create(downloadingTo);
                    fs.Close();
                }
...
new System.Threading.WaitCallback(this.WaitCallbackMethod), new object[] { urlList, "" });
        ...
        private void SaveToFile(byte[] buffer, int count, string fileName)
        {
            ...
            try
            {
                f = System.IO.File.Open(fileName, FileMode.Append, FileAccess.Write);
    ...
    class DownloadData
    {
        ...
        public static DownloadData Create(string url, string destFolder, IWebProxy proxy)
        {
            ...
            if (!downloadData.IsProgressKnown && System.IO.File.Exists(downloadTo))
